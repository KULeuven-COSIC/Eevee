...
def move_target_files(args, target):
    ...
    bytecode_dir = f'{args.mpspdzhome}/Programs/Bytecode/'
    mkdir(bytecode_dir)
    for path in bytecode_files:
        shutil.copy(path, bytecode_dir)
    schedule_file = run(f'ls -1 {target}.sch')
    schedule_dir = f'{args.mpspdzhome}/Programs/Schedules/'
    mkdir(schedule_dir)
    shutil.copy(schedule_file, schedule_dir)
...
def benchmark(args, target, playerid):
    ...
    with open(f'{args.mpspdzhome}/target', 'w') as fp:
        fp.write(target)
    # run prep script
    prep_cmd = f'sh {args.prep} > {as_logname(args.prep, playerid, dir=target)} 2>&1'
    print(f'Running prep script: {prep_cmd}')
    run(prep_cmd)
    print('\tDone')
    # run online script
    online_cmd = f'sh {args.online} > {as_logname(args.online, playerid, dir=target)} 2>&1'
    print(f'Running online script: {online_cmd}')
    run(online_cmd)
    print('\tDone')
    # run both script
...
parser.add_argument('--setup', dest='setup', required=False, type=str, help='Path to setup script', default='setup-experiment.sh')
parser.add_argument('--prep', dest='prep', required=False, type=str, help='Path to prep script', default='run-prep.sh')
parser.add_argument('--online', dest='online', required=False, type=str, help='Path to online script', default='run-online.sh')
...
if not args.skip:
    ...
    if not os.path.isfile(args.setup):
        raise Error(f'Cannot find setup script {args.setup}')
    if not os.path.isfile(args.prep):
        raise Error(f'Cannot find prep script {args.prep}')
    if not os.path.isfile(args.online):
        raise Error(f'Cannot find online script {args.online}')
...
# run setup script
print(f'Running {args.setup}')
run(f'sh {args.setup} > {as_logname(args.setup, playerid)} 2>&1')
print('\tDone')
...
if args.detach:
    ...
    cmd = f"python3 -u {sys.argv[0]} --skip --mp-spdz '{args.mpspdzhome}' --setup '{args.setup}' --prep '{args.prep}' --online '{args.online}' --both '{args.both}' {targets}"
