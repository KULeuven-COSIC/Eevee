...
class JolteonBase:
    ...
    def jolteon_dec(self, associated_data, ciphertext, tag, key, nonce):
        assert len(key) == 128
        ...
        if n_c_blocks > 0:
            if n_c_blocks > 1:
                # pack ciphertext except the last block
                ciphertext_packed = self._pack_blocks(ciphertext[:(n_c_blocks-1)*16])
                c_tweak = self._pack_tweak(nonce, self.ONE, n_c_blocks-1, 2, 2+n_c_blocks)
                # pack key
                c_key_packed = [self._pack_bit([keybit] * (n_c_blocks-1)) for keybit in key]
                if PRINT_DEBUG:
                ...
                message_packed = self.fk_dec(ciphertext_packed, c_key_schedule, 'i', '0')
                #try free key schedule
                try:
                    c_key_schedule.free()
                except AttributeError:
                ...
                for i in range(16):
                    ...
                    assert len(message_cells) == n_c_blocks-1
                    for k in range(n_c_blocks-1):
                        if k == 0:
                            cell = self._xor_cells([t_a[i], message_cells[k]])
                        else:
                            cell = self._xor_cells([ciphertext[16*(k-1) + i], message_cells[k]])
                        message[k][i] = cell
                    t_a[i] = self._xor_cells([t_a[i]] + message_cells)

            last_c_tweak = self._pack_tweak(nonce, self.ONE, 1, 2, 0 if complete_c else 1)
            last_c_key_schedule = self.fk.expand_key(key, last_c_tweak, self.cellsize, self.fk_params)
            m_star_prime, c_star_prime = self.fk_dec(tag, last_c_key_schedule, 'b', '0')

            last_block_length = len(ciphertext) % 16 if not complete_c else 16
            c_star = [None] * last_block_length

            for i in range(16):
                m_star_prime[i] = self._xor_cells([m_star_prime[i], t_a[i]])
                if i < last_block_length:
                    c_star[i] = self._xor_cells([m_star_prime[i], c_star_prime[i]])
                    message[-1][i] = m_star_prime[i]
        ...
        if n_c_blocks == 0:
            ...
            tag_correct = self._check_tag_eq(c_star, ciphertext[-last_block_length:])
            tag_correct = tag_correct * self._check_tag_eq(m_star_prime[last_block_length:], padding)
