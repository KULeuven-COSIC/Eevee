...
#define MAX_CIPHERTEXT_LEN 1616
...
int main(void)
{
    ...
    unsigned char *ciphertext = malloc(MAX_CIPHERTEXT_LEN * sizeof(unsigned char));
    if(ciphertext == NULL) {
      free(message);
      // stop with error
      while(1) {
    ...
    while(1){
        ...
        unsigned long long clen = clen_mimc128_ctr(message_len);
        ...
        int ret = htmac_mimc_128_crypto_aead_encrypt(ciphertext, clen+16, message, message_len, NULL, 0, nonce, key);
        ...
        // send ciphertext
        send_USART_bytes(ciphertext, clen);
        // send tag
        send_USART_bytes(ciphertext + clen, 16);
    }
